...
import { isObject, include, isEmptyObject, isString, decodeURIComponent, extend } from '../utils/index';
...
//获取自定义key的utm值
export function getCustomUtmFromQuery(query, utm_prefix, source_channel_prefix, exutm_prefix) {
  if (!isObject(query)) {
    return {};
  }
  var result = {};
...
export function getQueryUrl(params, isEncode) {
  ...
  if (params && isObject(params) && !isEmptyObject(params)) {
    ...
    each(params, function (value, key) {
      if (!(key === 'q' && isString(value) && value.indexOf('http') === 0) && key !== 'scene') {
        if (isEncode) {
          arr.push(key + '=' + value);
        } else {
